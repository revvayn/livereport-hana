...
exports.getRejectRateBlowdetector = async (req, res) => {
  try {
    const whereBD = `
      WHERE
        workcenter IS NOT NULL
        AND UPPER(TRIM(workcenter)) LIKE '%BLOW%DETECTOR%'
        AND valid_qty_pcs > 0
    `;
    ...
    console.error("RejectRateBlowdetector Error:", err);
    res.status(500).json({
      message: "Gagal mengambil data reject Blow Detector",
      error: err.message,
    });
  }
};
