...
export default function ItemRoutings() {
  ...
  const handleClearData = async () => {
    const confirm = await Swal.fire({
      ...
      text: "Tindakan ini akan menghapus SELURUH data Item Routing dan tidak dapat dibatalkan!",
      icon: "warning",
      showCancelButton: true,
    ...
    if (confirm.isConfirmed) {
      try {
        setLoading(true);
        await api.delete("/item-routings/clear-all");
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  ...
                  <th className="px-4 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Ass.Pannel</th>
                  <th className="px-4 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Ass.Core</th>
