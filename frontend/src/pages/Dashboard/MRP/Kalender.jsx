...
export default function Kalender({ itemId }) {
    ...
    const fetchSchedule = useCallback(async () => {
        ...
        try {
            ...
            const schedule = res.data.production_schedule || [];
            ...
            // Simpan data ke dalam kategori yang aktif
            setShiftData(prev => ({ ...prev, [activeTab]: formatted }));
        } catch (err) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const payload = Object.entries(currentData).map(([date, shifts]) => ({
                date,
                shifts: {
                    shift1: { qty: Number(shifts.shift1 || 0) },
                    shift2: { qty: Number(shifts.shift2 || 0) },
                    shift3: { qty: Number(shifts.shift3 || 0) },
                }
            }));

            await axios.patch(`/api/production/${itemId}/schedule`, { production_schedule: payload });
    ...
    const generateCalendar = (m, y) => {
        ...
        const startOffset = (firstDay + 6) % 7; // Penyesuaian agar mulai dari Senin
        const totalDays = new Date(y, m + 1, 0).getDate();
        const calendar = [];
        let currentDay = 1;
        
        for (let i = 0; i < 42; i++) {
            if (i < startOffset || currentDay > totalDays) {
                calendar.push(null);
            } else {
    ...
    return (
        <div className="max-w-6xl mx-auto p-4 sm:p-6 bg-gray-50 min-h-screen font-sans">
            ...
            <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100 relative">
                ...
                {/* Bagian Kontrol Navigasi (Bulan & Tahun) */}
                <div className="p-6 border-b border-gray-100 flex flex-col sm:row justify-between items-center gap-4">
                    <div className="flex items-center gap-4">
                        <div className={`p-3 rounded-2xl text-white shadow-md ${CATEGORIES.find(c => c.id === activeTab).color}`}>
                        ...
                        <div>
                            ...
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Unit: {activeTab}</p>
                        </div>
                    </div>
                                    ...
                                    {date && (
                                        <>
                                            <span className={`text-xs font-black w-7 h-7 flex items-center justify-center rounded-lg mb-2 
                                                ${isToday ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-50 text-gray-400 group-hover:text-blue-600'}`}>
                                                {date}
                                            </span>
